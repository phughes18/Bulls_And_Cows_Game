...
//             size - integer number of values in randomized computer array (four)
// PRECONDITION: The program has been initialized.
// POSTCONDITION: the computer array is filled with four different random values anywhere from one and nine.

void fill_array(int comp_values[], int size);


// Prints the randomized computer array at the beginning of game if the DEBUG compiling option is
// chosen by the player.
// PARAMETERS: comp_values[] - randomized computer integer array of values
//             size - integer number of values in randomized computer array (four)
// PRECONDITION: User selects the DEBUG compiling option.
// POSTCONDITION: the correct computer array is displayed by the progam before the player enters
//                their four number guess.

void print_array(const int comp_values[], const int size);


// Prompts player to enter their four number guess and creates a player array.
// PARAMETERS: player_guess[] - integer array of values entered by player when guessing computer array
//             size - integer number of values in player array
// PRECONDITION: four unique numbers are entered by the user.
// POSTCONDITION: an array is created containing the player's four number guess and it is displayed
//                back to the player.
...
// PARAMETERS: comp_values[] - randomized computer integer array of values
//             player_guess[] - integer array of values entered by player guessing computer array
...
// PRECONDITION: number of games played and number of total guesses are greater than zero.
...
//             worst_score - most integer number of guesses played in an individual game
//             total_guesses - total integer number of guesses in all session games combined
// PRECONDITION: number of games played and number of total guesses are greater than zero and user
//               has chosen to end game session.
...
void game_stats(const int game_number, const int best_score, const int worst_score, int total_guesses);
...
// PRECONDITION: games played is greater than zero and user has provided a one character input.
...
int main()  
{
    ...
    do
    {
        ...
        // Option to compile for debugging and use the print array function

        if (DEBUG)
            print_array(comp_values, TOTALNUMS);
        ...
        while (bulls < 4)
        {
            ...
            // Functions that prompt player to enter a guess and determines the number
        ...
        // Function to track game session stats
...
void fill_array(int comp_values[], int size)
{
    ...
    // do-while loop to continue generating random numbers until the boolean value 
    // is false and there are four non-duplicate numbers
...
// Function that prints the randomized computer array at the beginning of the game if the DEBUG
// compiler option is chosen by the player

void print_array(const int comp_values[], const int size)
{
    using namespace std;

    cout << "The randomized computer array is: ";
...
// Function that fills the player array with four values and displays
// array to the player
...
// Function that reads through the four values of each array and records matching array values in the 
...
// Function that updates the number of total guesses, guesses per game, number of games, best score and
// worst score of the session
